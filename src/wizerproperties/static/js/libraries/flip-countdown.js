  ...
  $.fn.flipper = function(action, options) {
      ...
      templateParts.forEach(function(part, index){
        ...
        if(part === 'dd' || part === 'ddd'){
          for(n = 0; n <= 9; n++){
            ...
            // $part.find('.flipper-digit:eq(1)').append('<div class="digit-face">' + n + '</div>');
            if(part === 'ddd'){
              var endDateTime = new Date(settings?.datetime);
              var now = new Date();
              var timeDifference = endDateTime - now;
              var days = Math.floor(timeDifference / (1000 * 60 * 60 * 24))?.toString();
              for (let i = 0; i < days.length; i++) {
                $part.find('.flipper-digit:eq('+i+')').append('<div class="digit-face">' + n + '</div>');
              }
            }
          }
        }
    ...
    function setFlipperDate($flipper, dateString, animate){
      ...
      if(dateString === 'now'){
        ...
        if(days < 0){
          remainder = 0 // if the duration finied
          return
        }
      ...
      var days_str = days.toString().padStart(days.length, '0');
      ...
      if(animate){
        ...
        // three sections
        for (let i = 0; i < days_str.length; i++) {          
          $flipper.find('.flipper-ddd').find('.flipper-digit:eq('+i+')').attr('data-value', days_str[i]);
        }
      }
      else {
        ...
        // three sections
        for (let i = 0; i < days_str.length; i++) {
          $flipper.find('.flipper-ddd .flipper-digit:eq('+i+') .digit-face:contains(' + days_str[i] + ')').addClass('active');
        }
