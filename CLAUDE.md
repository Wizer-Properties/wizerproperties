...
10. **New Tailwind classes silently don't render in production** - `wizerproperties-assets-1` only runs `npm run tailwind:build` once at container startup, then `sleep infinity`; it does NOT watch for template changes. Any Tailwind utility class used for the first time anywhere in the codebase (e.g. a new arbitrary value, a spacing/sizing class not used elsewhere) will be missing from the compiled `tailwind.css` and silently apply no styling at all - no error, just an unstyled element. This caused a badge icon to render at full unstyled size on 2026-07-08/09 (Home Helper AI redesign). Fix after adding any new Tailwind class: `docker restart wizerproperties-assets-1` (rebuilds CSS) then `docker restart wizerproperties-web-1` (re-runs collectstatic so Nginx serves the fresh build). Safer default: reuse classes already present elsewhere in the codebase, or use inline `style` attributes for anything novel, to avoid this rebuild step entirely.
